...
- 위키에 값이 "알 수 없음"으로 표기된 항목들은 0으로 수집되며, `validate_data.py` 실행 시 경고로 표시됩니다. 건물 25레벨 시간 7건과 연구 10레벨 시간 7건 등 18건은 외부 자료([riseofkingdomsguides.com](https://riseofkingdomsguides.com))와 동일 시간표를 공유하는 형제 연구의 값으로 `overrides.json`에서 보정했습니다. 남은 경고는 대장간 1레벨 전투력 1건뿐입니다(신뢰할 출처 없음, 총 전투력 표시에만 영향).
- 시청(City Hall) 24레벨의 전투력 수치는 위키 원문의 오타로 판단되어 `overrides.json`을 통해 보간한 값을 사용합니다.
- 위키가 선행 조건 칸을 비워 둔 22개 레벨(정찰대 캠프 7·8·10~25, 동맹 센터 7~9, 판금 갑옷 10)은 `overrides.json`에서 보정했습니다. 건물은 같은 항목의 다른 레벨이 모두 "시청 = 같은 레벨"을 요구하는 패턴에 맞춰 보간한 값이고, 판금 갑옷 10레벨은 위키 원문("Acedemy" 오타, 링크 없는 평문)을 그대로 옮긴 값입니다. 스크래핑 시 `unrecognized requirement cell` 경고로, `validate_data.py`에서는 선행 조건 연속성 경고로 드러납니다. 교역소 2~9레벨은 위키가 "-"(없음)로 명시해 그대로 둡니다.
- 자원 수급(생산량)은 모델링하지 않으며, 목표 달성에 필요한 자원 총량만 계산합니다.
- 특수 재화는 성(계약의 서)·경계탑(저항의 화살)과 모든 건물의 24→25 청사진만 반영하며, 보석 환산은 개당 계약의 서·저항의 화살 10개, 청사진 2,000개 기준입니다. 이 값들은 위키 스크래핑 대상이 아니라 `web/src/engine/materials.ts`에 직접 적어 둔 수치입니다.
...
- 가속은 아이템 개수가 아니라 종류별(범용/건설/연구) 총 보유 시간으로 입력하며, 작업마다 남은 시간만큼만 나눠 쓰는 것으로 계산합니다. 실제 게임에서는 아이템 단위로만 사용할 수 있어 큰 아이템을 쓸 때 초과분이 버려질 수 있으므로, 계산 결과가 실제보다 약간 유리하게 나올 수 있습니다.
